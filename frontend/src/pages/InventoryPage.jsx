...
export default function InventoryPage() {
  ...
  const handleUseAppearanceItem = (item) => {
    const currentActiveId = activeItems[item.subCategory];
    
    // Jika item yang sama diklik lagi, nonaktifkan (kembali ke default)
  ...
  // Logika untuk menambahkan item default ke setiap subkategori tampilan
  if (activeCategory === 'tampilan' && activeSubCategory !== 'semua') {
      const defaultItemsMap = {
          tema: { id: null, name: 'Tema Standar', icon: '☀️', category: 'tampilan', subCategory: 'tema' },
          avatar: { id: null, name: 'Avatar Standar', icon: '👤', category: 'tampilan', subCategory: 'avatar' },
          border: { id: null, name: 'Border Standar', icon: '⬜', category: 'tampilan', subCategory: 'border' },
          'gelembung chat': { id: null, name: 'Gelembung Standar', icon: '💬', category: 'tampilan', subCategory: 'gelembung chat' },
          'warna nama': { id: null, name: 'Warna Standar', icon: '🔤', category: 'tampilan', subCategory: 'warna nama' },
      };
      
      const defaultItem = defaultItemsMap[activeSubCategory];
      if (defaultItem && !filteredInventory.some(item => item.id === null)) {
          filteredInventory.unshift(defaultItem);
      }
  }
      ...
      {filteredInventory.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredInventory.map((item, index) => {
            ...
            let buttonText = isActive ? 'Digunakan' : 'Gunakan';
            let buttonAction = () => handleUseAppearanceItem(item);
            let buttonClass = isActive ? 'bg-green-500 text-white cursor-not-allowed' : 'bg-blue-500 text-white hover:bg-blue-600';

            return (
              <div key={`${item.id}-${index}`} className="relative bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 flex flex-col text-center items-center transform hover:scale-105 transition-transform duration-300">
                {isActive && <div className="absolute top-2 left-2 bg-green-500 text-white text-xs font-bold px-2 py-1 rounded-full z-10">✓</div>}
                ...
                {item.subCategory === 'warna nama' ? (() => {
                    const styleInfo = WARNA_NAMA_MAP[item.id] || WARNA_NAMA_MAP[null];
                    if (styleInfo.type === 'class') {
                        return <h3 className={`text-xl font-bold ${styleInfo.value}`}>{item.name}</h3>;
                    }
                    return <h3 className="text-xl font-bold" style={{ color: styleInfo.value || undefined }}>{item.name}</h3>;
                })() : (
                ...
                <button className={`mt-4 w-full py-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${buttonClass}`} onClick={buttonAction}>
                  {buttonText}
                </button>
              </div>
            );
